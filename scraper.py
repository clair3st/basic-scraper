...
def clean_data(td):
    """Clean data removing extra whitespace."""
    data = td.string
    try:
        return data.strip(" \n:-")
...
def extract_restaurant_metadata(elem):
    """Take in a restaurant and return dict containing metadata."""
...
def is_inspection_row(elem):
    """Get inspections data from table rows."""
    is_tr = elem.name == 'tr'
    if not is_tr:
        return False
...
def generate_results(test=False, count=10):
    """Generate health inspection results."""
    kwargs = {
        'Inspection_Start': '2/1/2007',
...
def get_geojson(result):
    """Build geojson dict using inspection data."""
